  ...
  return (
    <main className="h-full w-full flex flex-col content-center justify-center py-10">
      <div className="flex justify-end items-center gap-12 pr-10 mb-8 ml-8">
        <h1 className="text-6xl font-serif-extrabold">Foodly</h1>
          <SearchBar onSearch={handleSearch} />
            <button className="flex justify-between items-center px-4 py-5 h-6 w-40 bg-red-500 rounded-2xl text-white" onClick={showBookmarked} >
              <Icon icon="material-symbols:bookmark-sharp" className="w-7 h-7" />
              <span className="text-lg font-serif-extrabold">Bookmarks</span>
          </button>
      ...
      <div className="flex justify-center gap-8 pl-4">
        <div className="w-1/4 bg-red-500 text-white flex flex-col text-center justify-start rounded-lg p-3 h-screen">
            <h2 className="text-4xl font-serif-extrabold mt-4">Filters</h2>
            <div className="flex flex-col justify-start">
                {filterTags.map((tag) => (
                  <label
                    key={tag.slug}
                    className="flex items-center font-serif-extrabold my-1 text-1xl"
                  >
                    <input
                      type="checkbox"
        ...
        <div className="w-3/4">
          <div className="flex flex-wrap justify-start gap-8">
            {recipes.map((recipe) => (
                ...
                >
                  <h2 className="text-2xl font-serif-extrabold mb-2">{recipe.title}</h2>
          ...
          {selectedRecipe && (
                  <div className="fixed top-0 left-0 right-0 bottom-0 bg-gray-800 bg-opacity-40 flex justify-center items-center">
                    <div className="bg-white rounded-lg shadow-lg p-6 w-2/4 h-3/4 flex flex-col items-center justify-center overflow-y-scroll">
                      <h2 className="text-2xl font-bold mt-10">{selectedRecipe.title}</h2>
